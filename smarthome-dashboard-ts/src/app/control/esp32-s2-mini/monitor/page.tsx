...
export default function MonitorPage() {
  ...
  const connectMqtt = useCallback(() => {
    ...
    const c = mqtt.connect(BROKER_URL, { clientId: SENDER_CLIENT_ID, clean: true, reconnectPeriod: 4000 });
    clientRef.current = c;

    c.on('connect', () => {
      setConnecting(false); setError(null);
      // subscribe to status topics (retained gives immediate last state → presence)
      c.subscribe(STATUS_TOPICS, { qos: 0 }, (err) => {
        if (err) {
          setError('Subscribe error: ' + err.message);
        }
      });
      // also watch $SYS/broker/clients/connected for coarse presence (optional)
      c.subscribe('$SYS/broker/clients/connected');
      // subscribe presence
      c.subscribe(PRESENCE_TOPIC);
    });

    c.on('message', (topic, payloadBuf, packet) => {
    ...
    c.on('error', (e) => { setError(e.message); });
    c.on('close', () => { setConnecting(true); });
  }, []);

  useEffect(() => { connectMqtt(); return () => { clientRef.current?.end(true); }; }, [connectMqtt]);
