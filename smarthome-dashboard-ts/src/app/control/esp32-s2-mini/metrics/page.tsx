...
export default function BrokerMetricsPage() {
  ...
  useEffect(() => {
    let cancelled = false;
    const c = mqtt.connect(MQTT_URL, { reconnectPeriod: 3000, clean: true, connectTimeout: 10_000 });
    setClient(c);
    c.on('connect', () => { 
      if (cancelled) return; 
      setStatus('online');
      // Subscribe only after successful connection to avoid 'Connection closed' volatile flush errors
      const topics: string[] = ['$SYS/#','smarthome/#'];
      const attempt = () => {
        if (cancelled) return;
        c.subscribe(topics, (err) => {
          if (err) {
            console.error('Subscribe failed, retrying...', err);
            setTimeout(() => { if (c.connected) attempt(); }, 1500);
          }
        });
      };
      attempt();
    });
    c.on('reconnect', () => { if (!cancelled) setStatus('connecting'); });
    c.on('close', () => { if (!cancelled) setStatus('offline'); });
    c.on('error', () => { if (!cancelled) setStatus('error'); });
    ...
    c.on('message', (topic, payloadBuf) => {
      ...
      // integrity sequence + rolling hash (very simple XOR+mix)
      seqRef.current += 1;
      setIntegrity(prev => {
      ...
      if (typeof bytesReceived === 'number' || typeof bytesSent === 'number') {
        ...
        if (lastBytes != null && lastTime) {
          const dt = (now - lastTime) / 1000;
            ...
            setSampleBytesPerSec(prev => {
              const arr = [...prev, Math.max(0, Math.round(rate))];
    ...
    return () => { cancelled = true; clearInterval(interval); try { c.end(true); } catch {} };
  }, []);
