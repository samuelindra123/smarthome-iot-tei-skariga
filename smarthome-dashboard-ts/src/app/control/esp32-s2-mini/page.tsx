...
export default function ControlEsp32S2MiniPage() {
  ...
  useEffect(() => {
    ...
    mqttClient.on('reconnect', () => { if (!isCancelled) { console.log('Mencoba reconnect ke MQTT...'); setReconnecting(true); setConnected(false);} });
    ...
    return () => {
      isCancelled = true;
      try { mqttClient.end(true); } catch (e) { console.warn('Cleanup MQTT error:', e); }
    };
  }, []);
